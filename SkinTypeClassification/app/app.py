...
async def validate_image(image_path):
    ...
    try:
        ...
        # Create skin masks from both color spaces
        ...
        # Lower threshold (5% instead of 10%) since we're accepting any skin part, not just face
...
class SkinTypeResponse(BaseModel):
    skin_type: str
    short_info: str
    skin_percentage: float  # Added to show how much skin was detected
...
def skin_color_analysis(image_path):
    ...
    # Calculate average hue and saturation in skin areas
    # Use the skin mask we already have
    ycrcb = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)
    ...
    # Get average values where skin is detected
    ...
    # Calculate red channel intensity (sensitive skin often has more redness)
    ...
    # Higher red values and lower saturation can indicate sensitive skin
...
async def analyze_skin(file: UploadFile):
    ...
    try:
        ...
        # Validate the image has enough skin
        await validate_image(temp_image_path)
        
        # Calculate skin percentage for informational purposes
        ...
        try:
            ...
            # Complete the skin type classification
            ...
            # Validate the skin type is in our text info
